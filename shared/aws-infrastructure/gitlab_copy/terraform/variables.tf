...
variable "key_name" {
    description = "ID element. Usually the component or solution name, e.g. 'app' or 'gitlab'.This is the only ID element not also included as a tag. The name tag is set to the full id string. There is no tag with the value of the name input."
    type        = string
}
...
variable "gitlab_private_ips" {
  description = "gitlab private ips"
  type        = map(string)
}
