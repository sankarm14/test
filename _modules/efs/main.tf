...
resource "aws_efs_file_system" "default" {
  ...
  count                           = local.enabled && length(var.names) > 0 ? length(var.names) : 0
  tags = {
    Name                          = format("%s-%s", module.this.id, var.names[count.index])
  }
...
resource "aws_efs_mount_target" "default" {
  count             = local.enabled ? 4 : 0
  file_system_id    = join("", aws_efs_file_system.default[count.index].*.id)
