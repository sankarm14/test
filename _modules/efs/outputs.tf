...
output "arn" {
  value       = local.enabled ? aws_efs_file_system.default[*].*.arn : null
  description = "EFS ARN"
}

output "id" {
  value       = local.enabled ? aws_efs_file_system.default[*].*.id : null
  description = "EFS ID"
}

output "efs_availability_zone" {
  value = aws_efs_file_system.default.0.availability_zone_name
}
