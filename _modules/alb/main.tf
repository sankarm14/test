...
resource "aws_security_group_rule" "https_ingress" {
  count             = module.this.enabled && var.security_group_enabled && var.https_enabled ? 2 : 0
  type              = "ingress"
...
module "default_target_group_label" {
  source          = "cloudposse/label/null"
  version         = "0.25.0"
  attributes      = concat(module.this.attributes, [""])
  id_length_limit = var.target_group_name_max_length
  context         = module.this.context
}

resource "aws_lb_target_group" "defaults" {
  count                = module.this.enabled && var.default_target_group_enabled && length(var.target_group_ports) > 0 ? length(var.target_group_ports): 0
...
#  count             = module.this.enabled && var.http_enabled && var.http_redirect != true ? 2 : 0
#  load_balancer_arn = join("", aws_lb.default[count.index].*.arn)
#  port              = var.http_port
#  protocol          = "HTTP"
#  tags              = merge(module.this.tags, var.listener_additional_tags)
#
#  default_action {
#    target_group_arn = var.listener_http_fixed_response != null ? null : join("", aws_lb_target_group.default[count.index].*.arn)
...
#
#  default_action {
#    target_group_arn = join("", aws_lb_target_group.default[count.index].*.arn)
#    type             = "redirect"
#
#    redirect {
#      port        = var.https_port[count.index]
#      protocol    = "HTTPS"
...
resource "aws_lb_listener" "https" {
  #bridgecrew:skip=BC_AWS_GENERAL_43 - Skipping Ensure that load balancer is using TLS 1.2.
  count             = module.this.enabled && var.https_enabled && length(var.https_ports) > 0 ? length(var.https_ports) : 0
  ...
  default_action {
    target_group_arn = var.listener_https_fixed_response != null ? null : join("", aws_lb_target_group.defaults[count.index].*.arn)
...
#  listener_arn    = join("", aws_lb_listener.https[count.index].*.arn)
